...
import sys
from utils import *
from models import EEGEncoder_nochunked
import csv
import os.path
...
# chunk_size = 10 # X_trainval.shape[-1]
...
# - loss function
# - GPU
datalength = torch_X_train.shape[2]
model_EEGEncoder = EEGEncoder_nochunked(input_size = len(electrodes), datalength = datalength)
    ...
    for epoch in range(N_EPOCHS):
        ...
        if epoch > 500 and valid_loss < best_valid_loss:
            ...
            try:
                ...
                pass
            torch.save(model.state_dict(), "../model/03_FeatureExtraction/{par}/{roundno}/{electrode_zone}/{task}/EEG_ENCODER_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked.pt.tar".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax))
            best_model_index = i
...
classes = np.array(('Red', 'Green', 'Blue'))
model = EEGEncoder_nochunked(input_size = len(electrodes), datalength = datalength)
model = model.float()
model = model.to(device)
model.load_state_dict(torch.load("../model/03_FeatureExtraction/{par}/{roundno}/{electrode_zone}/{task}/EEG_ENCODER_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked.pt.tar".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax)))
...
# Save Train
if psd_enable:
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/X_train_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),X_train)
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/y_train_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),y_train)

    # Save Test
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/X_test_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),X_test)
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/y_test_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),y_test)

    # Save Val
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/X_val_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),X_val)
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/y_val_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),y_val)

    # Save Extracted Features
    np.save('../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/extracted_features_X_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked'.format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax), eeg_extracted_features )
    np.save('../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/extracted_features_y_{fmin}_{fmax}_{tmin}_{tmax}_psd_nochunked'.format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax), y_train_val)

else:
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/X_train_{fmin}_{fmax}_{tmin}_{tmax}_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),X_train)
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/y_train_{fmin}_{fmax}_{tmin}_{tmax}_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),y_train)

    # Save Test
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/X_test_{fmin}_{fmax}_{tmin}_{tmax}_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),X_test)
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/y_test_{fmin}_{fmax}_{tmin}_{tmax}_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),y_test)

    # Save Val
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/X_val_{fmin}_{fmax}_{tmin}_{tmax}_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),X_val)
    np.save("../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/y_val_{fmin}_{fmax}_{tmin}_{tmax}_nochunked".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax),y_val)

    # Save Extracted Features
    np.save('../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/extracted_features_X_{fmin}_{fmax}_{tmin}_{tmax}_nochunked'.format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax), eeg_extracted_features )
    np.save('../data/participants/{par}/03_FeatureExtraction/{roundno}/{electrode_zone}/{task}/extracted_features_y_{fmin}_{fmax}_{tmin}_{tmax}_nochunked'.format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax), y_train_val)
...
filename = "../results/classification_results_psd_nochunked.csv"
mode = 'a' if os.path.exists(filename) else 'w'
with open(f"../results/classification_results_psd_nochunked.csv", mode) as myfile:
