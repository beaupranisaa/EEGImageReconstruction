    ...
    for epoch in range(N_EPOCHS):
        ...
        if epoch > 500 and valid_loss < best_valid_loss:
            ...
            try:
                ...
                pass
            torch.save(model.state_dict(), "../model/03_FeatureExtraction/{par}/{roundno}/{electrode_zone}/{task}/EEG_ENCODER_{fmin}_{fmax}_{tmin}_{tmax}_psd.pt.tar".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax))
            best_model_index = i
...
model = model.float()
model = model.to(device)
model.load_state_dict(torch.load("../model/03_FeatureExtraction/{par}/{roundno}/{electrode_zone}/{task}/EEG_ENCODER_{fmin}_{fmax}_{tmin}_{tmax}_psd.pt.tar".format(par=par,task=task,roundno=roundno,electrode_zone=electrode_zone,fmin=fmin,fmax=fmax, tmin=tmin, tmax=tmax)))
...
filename = "../results/classification_results_psd.csv"
mode = 'a' if os.path.exists(filename) else 'w'
with open(f"../results/classification_results_psd.csv", mode) as myfile:
